...
docker-run-production:  ## Runs production server in docker
	python3 manage.py migrate
	gunicorn club.asgi:application -w 4 -k uvicorn.workers.UvicornWorker --bind=0.0.0.0:8814 --capture-output --log-level debug --access-logfile - --error-logfile -
...
psql:
	psql -h localhost -p 5433 -d rationalanswer -U ra
