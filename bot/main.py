...
def private_message(update: Update, context: CallbackContext) -> None:
    ...
    if str(update.effective_user.id) not in set(club_users):
        update.effective_chat.send_message(
            "Привет! Мы пока не знакомы. Привяжите меня к аккаунту командой /auth с "
            "<a href=\"https://rationalanswer.club/user/me/edit/bot/\">кодом из профиля</a> через пробел",
            parse_mode=ParseMode.HTML
        )
    else:
        update.effective_chat.send_message(
...
def main() -> None:
    ...
    updater = Updater(
        settings.TELEGRAM_TOKEN, use_context=True, request_kwargs=_request_kwargs
    )
